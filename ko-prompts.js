 ...
 * ══════════════════════════════════════════════════════════════════
 *  Version: 2.54.4 (23.09.2026) — "ABSCHNITT-7+8-TEMPLATING" (Backlog-Punkt
 *  vom 13.09.2026, heute aufgegriffen: "Boilerplate-Abschnitte 7+8 ... sind
 *  grossteils statisch ... sollen app-seitig templatet statt vom LLM
 *  erzeugt werden — spart Input+Output-Token", direkte Fortsetzung des
 *  heutigen Kostensenkungs-Themas nach Prompt Caching v2.54.3).
 *
 *  BEFUND (empirisch belegt, nicht vermutet — alle 15 Strategien, je zwei
 *  stark unterschiedliche Tageskontexte getestet): Abschnitt 7 ("Was UIQ
 *  ableiten kann") und Abschnitt 8 ("Modell-Grenze") sind byte-identisch
 *  unabhaengig von ctx.marktkontext/ctx.top3Syms — einzige Eingaben sind
 *  o.stratName, o.modellGrenzeText, o.istOptionsStrategie, allesamt
 *  strategie-konstante Werte. validateBriefingCompliance() prueft
 *  nachweislich nichts aus Abschnitt 7/8 (nur Optionsmarkt-Validierungs-
 *  status, Bollinger/Tightness bei atmna, Ticker-Konsistenz in Abschnitt 3)
 *  — keine Kollision mit dieser Woche gehaerteten Compliance-Checks.
 *
 *  FIX: neue Funktion `_buildAbschnitt78(o)` — identischer Wortlaut wie
 *  bisher, nur aus der Inline-Logik in `_publicNinePointPrompt()`
 *  ausgelagert (keine Textduplikation). Neues, bewusst OPT-IN
 *  `ctx.skipAbschnitt78`-Flag: Default (Flag nicht gesetzt) = 100%
 *  unveraendertes Verhalten fuer JEDEN bestehenden Aufrufer (Browser,
 *  sonstige Skripte) — `.get()`/`.prompt()` liefern weiterhin einen reinen
 *  String mit Abschnitt 7+8 wie immer. NUR bei explizit gesetztem Flag
 *  (aktuell nur von generate_public_recommendations.js genutzt): Abschnitt
 *  7+8 entfallen im an die KI gesendeten Prompt UND in der erwarteten
 *  Modell-Ausgabe (neuer Uebergangshinweis erklaert dem Modell, direkt von
 *  Abschnitt 6 zu Abschnitt 9 zu springen, Nummerierung bleibt "9." nicht
 *  "7."), SCHLUSS-SELBSTPRÜFUNG SCHRITT 1 scannt entsprechend nur noch
 *  "Abschnitte 4, 5, 6 und 9" statt "4 bis 9". Rueckgabewert aendert sich
 *  in diesem Fall von String zu Objekt `{prompt, abschnitt7, abschnitt8}`
 *  — der aufrufende Code fuegt Abschnitt 7+8 NACH dem API-Call selbst
 *  wieder ein (deterministisch, kein LLM-Beitrag mehr fuer diese zwei
 *  Abschnitte).
 *
 *  ZUSATZFUND beim Testen (unabhaengiger, vorbestehender Bug, nicht durch
 *  diese Aenderung verursacht, aber dadurch aufgedeckt): `value.prompt(ctx)`
 *  baute intern ein KOMPLETT NEUES ctx-Objekt (nur `marktkontext` kopiert)
 *  statt das Original zusammen mit allen anderen Feldern durchzureichen —
 *  dadurch ging bislang JEDES andere ctx-Feld verloren, sobald `value`
 *  aufgerufen wurde (nicht nur das neue skipAbschnitt78-Flag). Gefixt:
 *  `Object.assign({}, ctx, {marktkontext: ...})` statt Neubau. Scope dieses
 *  Fixes unklar rueckwirkend (welche ctx-Felder `value` bisher sonst noch
 *  gefehlt haben, ausser dem hier getesteten) — kein Blocker, aber
 *  erwaehnenswert bei kuenftigen value-spezifischen Funden.
 *
 *  Noch KEIN Live-Test dieser Version — naechster Handelstag-Lauf sollte
 *  pruefen, ob das Modell den Uebergangshinweis (direkt von 6 zu 9
 *  springen) zuverlaessig befolgt, und ob die Wortzahl-Ersparnis im
 *  AI-Budget-Log sichtbar wird.
 *
 *  Version: 2.54.3 (23.09.2026) — "PROMPT-CACHING-VORAUSSETZUNG" (Axel:
 *  "ich MUSS Kosten kontrollieren", dringlich). Kein Verhaltenseingriff an
 *  den Prompt-Texten selbst — reine Export-Ergaenzung, damit
 *  `generate_public_recommendations.js` Anthropics Prompt-Caching nutzen
 *  kann (bis zu 90% Rabatt auf gecachte Input-Token, s. offizielle
 *  Anthropic-Doku platform.claude.com/docs/en/about-claude/pricing).
 *
 *  BEFUND: der bisherige API-Call in generate_public_recommendations.js
 *  sendet den kompletten Prompt als einzelnen String, keine cache_control-
 *  Markierung. Live vermessen (23.09.2026): `KI_ANTI_HALLUZINATION +
 *  PUBLIC_REGULATORY_GUARDRAIL` steht byte-identisch am Anfang ALLER 15
 *  Strategien (Public-Modus) — empirisch verifiziertes gemeinsames
 *  Praefix ueber alle 15 Strategien: 28.854 Zeichen / ~7.214 Token, weit
 *  ueber der Cache-Mindestgroesse. Ein einziger Cache-Write pro
 *  Nachtlauf koennte damit potenziell alle 15-21 Calls dieser Nacht
 *  bedienen, nicht nur wiederholte Calls derselben Strategie
 *  (Repair-Loop).
 *
 *  FIX: `PUBLIC_REGULATORY_GUARDRAIL` (bisher nur intern) sowie die neue,
 *  kombinierte `SHARED_STATIC_PREFIX`-Property exportiert — beide
 *  NAMENTLICH benannt statt einer hart codierten Zeichenzahl, damit die
 *  Cache-Grenze nicht bricht, sobald sich kuenftig der Wortlaut einer
 *  einzelnen Strategie (o.rolle etc.) aendert. Bewusst KEINE Aenderung an
 *  Abschnitt 1-9, REASONING-GUARDRAILS oder sonstigen Prompt-Inhalten —
 *  die tatsaechliche Umsortierung von `ctx.marktkontext`/
 *  `_deterministicOptionsFactBlock()` (fuer eine zweite, strategie-
 *  spezifische Cache-Ebene) ist NICHT Teil dieser Version, da das echte
 *  Aenderungen an der Reihenfolge der Modell-Instruktionen waere und vor
 *  einem Rollout einen eigenen Live-Test braucht (Grundgesetz #9 —
 *  Reihenfolge-Aenderungen an dieser Datei hatten in dieser Codebase
 *  wiederholt zu Live-Regressionen gefuehrt, zuletzt bei den v2.53.x/
 *  v2.54.x-Funden dieser Woche). Noch KEIN Live-Test dieser Version.
 *
 *  Version: 2.54.2 (22.09.2026) — "ABSCHNITTSGRENZEN-REGEX-FIX" (P0,
 *  ROOT CAUSE, vierter Live-Test-Anlauf — erster mit echtem Diagnose-
 *  Logging in generate_public_recommendations.js v1.17, dadurch endlich
 *  BELEGT statt weiter vermutet). Befund: _extractSection3Tickers()
 *  scheiterte bei ALLEN fünf Options-Strategien, in JEDEM geloggten Fall
 *  (Erstversuch UND Repair), bereits an der Abschnittsgrenzen-Erkennung
 *  ("Abschnitt-3-Grenze gefunden: false") — die eigentliche Ticker-
 *  Extraktion wurde nie erreicht.
 *
 *  ROOT CAUSE: das Modell schreibt Markdown-Überschriften standardmäßig
 *  komplett fett, INKLUSIVE der Abschnittsnummer — "**1. MARKT-/REGIME-
 *  KONTEXT**", nicht "1. MARKT-/REGIME-KONTEXT" (Beleg aus dem Live-Log:
 *  exakt dieses Muster für Abschnitt 1 sichtbar, Abschnitt 3 vermutlich
 *  identisch formatiert). Der bisherige Regex
 *  `(?:^|\n)\s*3\.[^\n]*\n(...)` erwartet nach dem Zeilenumbruch
 *  ausschließlich Whitespace vor der Ziffer — `\s*` matcht kein `*`
 *  (kein Whitespace-Zeichen) — wodurch der Match bei "**3. HÖCHSTE ...**"
 *  IMMER scheiterte, unabhängig davon, ob die Ticker im Abschnitt selbst
 *  korrekt fett markiert waren. Sehr wahrscheinlich der URSPRÜNGLICHE,
 *  primäre Bug seit Einführung dieses Validators (v2.53.30, 21.09.2026) —
 *  der v2.54.1-Fettdruck-Fix für Ticker (22.09.2026) war berechtigt und
 *  notwendig, konnte aber gar nicht wirken, weil die Funktion bereits vor
 *  der Ticker-Suche scheiterte.
 *
 *  FIX: Regex toleriert jetzt 0-2 Sternchen (optionale Markdown-Fettung
 *  der GESAMTEN Überschriftenzeile) direkt vor der Abschnittsnummer,
 *  sowohl für die Start- (Abschnitt 3) als auch die Ende-Grenze
 *  (Abschnitt 4). Bewusst als Regex-Robustheit gelöst, NICHT als weitere
 *  Prompt-Anweisung ("schreibe die Überschrift nicht fett") — dem Modell
 *  zu verbieten, Markdown-Überschriften markdown-typisch zu formatieren,
 *  wäre ein fragiler Kampf gegen dessen natürliches Standardverhalten,
 *  dieselbe Lehre wie bei zahlreichen anderen Guardrails in dieser Datei:
 *  eine deterministische Prüfung, die mit der natürlichen Modellausgabe
 *  umgehen kann, ist robuster als eine weitere Verhaltensvorschrift.
 *
 *  SCOPE: nur _extractSection3Tickers() geändert — keine Änderung an der
 *  Ticker-Fettdruck-Pflicht selbst (v2.54.1 bleibt inhaltlich gültig und
 *  notwendig, jetzt aber tatsächlich wirksam, da die Funktion die
 *  Ticker-Suche überhaupt erst erreicht). Analoge Korrektur in
 *  generate_public_recommendations.js::_diagnoseSection3() (v1.17s
 *  Diagnose-Logging spiegelt dieselbe Regex) separat in v1.18 nachgezogen
 *  — sonst würde das Diagnose-Logging ab jetzt fälschlich weiterhin
 *  "Grenze gefunden: false" zeigen, obwohl der echte Validator hier
 *  bereits repariert ist. NOCH KEIN erneuter Live-Test.
 *
 *  Version: 2.54.1 (22.09.2026) — "FETTDRUCK-VALIDIERUNGS-FIX" (P0,
 *  aus dem allerersten echten Live-Test des kompletten Compliance-
 *  Mechanismus — Batch-API v1.14 + Candidate-Selection-Integrity v2.54.0
 *  liefen heute zum ersten Mal zusammen scharf. Ergebnis: ALLE FÜNF
 *  Options-Strategien (csp_wheel/atmna/weekly_income/cc/collar) scheiterten
 *  an "top3-ticker-konsistenz", Repair-Versuch schlug ebenfalls fehl, v1.12-
 *  Publish-Fix griff korrekt (nichts Nicht-Konformes veröffentlicht) — aber
 *  eben GAR NICHTS wurde veröffentlicht, obwohl die Kandidatenauswahl selbst
 *  (per [ATMNA-CANDIDATES]-Diagnose-Log bestätigt: top3Syms korrekt) sauber
 *  lief).
 *
 *  ROOT CAUSE (verifiziert, nicht vermutet — gegen den tatsächlichen Prompt-
 *  Text geprüft, nicht nur den Code-Kommentar): _extractSection3Tickers()
 *  (v2.53.30, 21.09.2026) extrahiert AUSSCHLIESSLICH fett markierte Ticker
 *  ("**JNJ**") aus Abschnitt 3 — der zugehörige Kommentar behauptete, das
 *  sei "laut Abschnitt-3-Instruktion oben, s. dort" vorgegeben. War es
 *  NICHT: die tatsächliche PFLICHT-FORMULIERUNG in Abschnitt 3 (weder scan-
 *  noch holding_review-Zweig) verlangte nirgends Markdown-Fettdruck für die
 *  genannten Ticker — reine Doku-Fiktion, nie live getestet (sowohl dieser
 *  Changelog-Eintrag v2.53.30 als auch der zugehörige generate_public_
 *  recommendations.js-Eintrag v1.10 sagten explizit "noch KEIN Live-Test").
 *  Folge: _extractSection3Tickers() lieferte bei JEDEM Lauf ein leeres
 *  Array (keine Fettdruck-Marker im Modell-Output vorhanden), wodurch
 *  top3-ticker-konsistenz IMMER fehlschlug — unabhängig davon, ob die
 *  genannten Ticker inhaltlich korrekt waren. Derselbe Fehler steckte im
 *  Repair-Prompt (generate_public_recommendations.js::buildRepairPrompt()):
 *  auch dort keine Fettdruck-Anweisung, weshalb der Repair-Versuch
 *  strukturell nie erfolgreich sein konnte.
 *
 *  FIX (zwei Stellen in dieser Datei, Repair-Prompt-Fix separat in
 *  generate_public_recommendations.js v1.15):
 *  (1) Abschnitt 3, BEIDE Zweige (scan UND holding_review): PFLICHT-
 *  FORMULIERUNG explizit um die Fettdruck-Anforderung erweitert ("JEDES
 *  Tickersymbol in Markdown-Fettdruck, z.B. **AAPL** — technisch
 *  erforderlich für eine automatisierte Prüfung, kein gestalterischer
 *  Vorschlag"), Beispielsatz selbst zeigt jetzt **[Titel 1]**-Format statt
 *  unformatiertem [Titel 1] — im scan-Zweig NUR für istOptions=true (die
 *  Compliance-Prüfung betrifft ausschließlich die fünf Options-Strategien,
 *  Equity-Strategien bekommen weiterhin die unformatierte Variante, um
 *  keine unnötige neue Formatierungsauflage für einen dort nie geprüften
 *  Fall einzuführen).
 *  (2) SCHLUSS-SELBSTPRÜFUNG SCHRITT 3 (istOptions) um Prüfpunkt (c)
 *  erweitert — explizite Vor-Abgabe-Prüfung, ob jedes in Abschnitt 3
 *  genannte Tickersymbol tatsächlich fett markiert ist, mit dem heutigen
 *  Fund als dokumentiertem Beleg (analog zum bewährten Verstärkungsmuster
 *  bei Schritt 1/2: eine Regel allein reicht bei diesem Codebase
 *  wiederholt belegt nicht aus, eine explizite Vor-Abgabe-Prüfung erhöht
 *  die Trefferquote).
 *
 *  SCOPE: betrifft nur Abschnitt 3 und die SCHLUSS-SELBSTPRÜFUNG der fünf
 *  Options-Strategien — keine Änderung an der Candidate-Selection-Integrity-
 *  Architektur selbst (v2.54.0 bleibt inhaltlich unverändert gültig).
 *  NOCH KEIN erneuter Live-Test — nächster Lauf sollte gezielt prüfen, ob
 *  eine einzelne explizite Anweisung + Selbstprüfung reicht, oder ob (wie
 *  bei mehreren früheren Guardrails in diesem Changelog) eine zweite
 *  Verstärkungsrunde nötig wird, falls das Modell die Fettdruck-Vorgabe
 *  weiterhin nicht zuverlässig befolgt.
 *
 ...
 *  und der daraus gebaute deterministische Faktor-Block waren nachweislich
 *  korrekt (JNJ/MRK/MDT mit echten BB-/Tightness-Werten) — das Modell
 *  ersetzte in Abschnitt 3 dennoch eigenmächtig MRK durch CSCO und
 *  begründete aktiv, warum MRK "die Kriterien nicht erfüllt". Kein
 *  Datenfluss-Bug, sondern freie Modell-Neuinterpretation der Top-10-Liste
 *  trotz bereits abgeschlossener Score-Rang-Entscheidung. Fix, dreiteilig:
 *  (1) _deterministicOptionsFactBlock() bekommt eine neue Pflichtzeile
 *  "VERBINDLICHE TOP-3-KANDIDATEN FÜR ABSCHNITT 3" (aus o.top3Syms, für
 *  ALLE fünf Options-Strategien, nicht nur atmna); (2) Abschnitt-3-
 *  Instruktion verbietet explizit den Ticker-Austausch und das Ausschließen
 *  eines vorgegebenen Kandidaten; (3) validateBriefingCompliance() bekommt
 *  einen neuen optionalen dritten Parameter options.expectedTop3 —
 *  Mengenvergleich (nicht reihenfolgesensitiv) zwischen erwarteten und in
 *  Abschnitt 3 tatsächlich genannten Tickern (_extractSection3Tickers()).
 *  Rückwärtskompatibel: ohne expectedTop3 wird der neue Check komplett
 *  übersprungen. Architekturprinzip fest verankert: die Score-Engine
 *  entscheidet, das Modell erklärt — es darf die Kandidatenauswahl nicht
 *  ersetzen. Noch KEIN Live-Test.
 *
 ...
 *  BEIDE ERSATZLOS, obwohl Abschnitt 3/4 sie ausdrücklich verlangen — reine
 *  Regel-Existenz reichte nicht (bekanntes Muster bei neuen Guardrails, s.
 *  frühere Kausalitäts-/Zeitreihen-Sprache-Fixes). Guardrail i) (IVP/Grade-
 *  Divergenz) griff nur TEILWEISE — Abschnitt 9 benannte die Divergenz, aber
 *  mit falscher "teilt"-Sprache bei tatsächlich unterschiedlichen Werten
 *  (PJP 84%ile vs. JNJ 87%ile als "teilt den höchsten IVP-Bereich"
 *  bezeichnet — verstößt bereits gegen die bestehende Regel b, war aber
 *  nicht explizit genug verankert). ZUSÄTZLICH vom Reviewer gefunden: IVP
 *  fälschlich als "realisierte Volatilität" bezeichnet (ist echtes IV-
 *  Perzentil, kein HVP-Proxy) — Ursache vermutlich fehlende Feld-Legende im
 *  Options-Pfad (buildOptionsMarktkontext() in generate_public_
 *  recommendations.js liefert anders als der Equity-Pfad keine DATA_LEGENDE
 *  mit); sowie die berechnete Strike-Näherung (EMA200-1,5×ATR) fälschlich
 *  als "Rollschwelle" bezeichnet, obwohl die tatsächliche Rollentscheidung
 *  laut Ludwig-Prinzip zeit-/ökonomiebasiert ist, nicht kursabstandsbasiert.
 *  VIER FIXES: (1) neue BEGRIFFS-KLARSTELLUNG PFLICHT in Abschnitt 4
 *  (istOptions-generisch): IVP MUSS als implizite Volatilität bezeichnet
 *  werden, niemals als realisiert/historisch (Fallback auf HVP explizit
 *  kennzeichnen); EMA200-1,5×ATR MUSS als "modelliertes Referenzniveau"
 *  bezeichnet werden, niemals als "Rollschwelle"/"Puffer zur Rollschwelle".
 *  (2) Guardrail i) um explizite Ergänzung erweitert: "teilt"/"gemeinsam"
 *  bleibt ausschließlich echten Zahlen-Gleichständen vorbehalten (Regel b
 *  gilt unverändert), mit dem PJP/JNJ-Fund als belegtes Gegenbeispiel.
 *  (3) NEU: SCHRITT 3 in der SCHLUSS-SELBSTPRÜFUNG, conditional auf
 *  istOptions — explizite Vor-Abgabe-Prüfung, ob beide Pflicht-Zusätze aus
 *  Abschnitt 3/4 tatsächlich in der fertigen Antwort stehen (analog zum
 *  bewährten Muster von Schritt 1/2 für Ticker-Scope/Zeitreihen-Sprache,
 *  die ebenfalls erst nach mehreren Funden robust wurden). NOCH NICHT
 *  UMGESETZT (bewusst zurückgestellt, kein Scope-Sprengen kurz vor
 *  Feierabend): Terminologie-Vereinheitlichung "Strategy Fit" →
 *  "Kriterienübereinstimmung" (Reviewer-Vorschlag, betrifft "STRATEGY-FITS"-
 *  Überschrift systemweit in beiden Buildern, größerer Umbau), Abschnitt-9-
 *  Tabellenformat statt Mini-Ranking-Prosa, "Top-15"-Frontend-Wortlaut
 *  (liegt vermutlich in index.html/generate_public_recommendations.js, nicht
 *  hier), sowie eine echte Options-DATA_LEGENDE analog zum Equity-Pfad.
 *  Nächster Schritt: erneuter Live-Test, insbesondere ob Schritt 3 die
 *  beiden Pflicht-Zusätze jetzt zuverlässig erzwingt.
 *
 *  Version: 2.53.27 (18.09.2026) — ATMNA-EXPLAINABILITY-GAP-FIX
 *  (Backlog-Fund aus generate_public_recommendations.js v1.5,
 *  Thema-3-Abschlusstest 16.09.2026, heute wie geplant in der naechsten
 *  gezielten Options-Prompt-Qualitaetsrunde nachgezogen, direkt im
 *  Anschluss an den Reviewer-Fix v2.53.26). Befund: Score/Pipeline fuer
 *  atmna bevorzugen korrekt neutrale/stabile Setups ueber bbPos
 *  (Bollinger-Mittelzone) und tightnessPct — genau die beiden Felder,
 *  die atmna von den vier anderen Options-Strategien unterscheiden (s.
 *  OPTIONS_STRATEGY_SIGNAL_MAP) — aber der Public-Output erklaerte
 *  Rankings bisher ueberwiegend ueber generische Faktoren (RSI/Grade/
 *  D200/IVP), bbPos/tightnessPct blieben implizite Rohdaten im Ticker-
 *  Kontext statt explizite Ranking-Begruendung. KEIN Score-/Pipeline-
 *  Fehler. FIX: neuer generischer Hook `o.expliziteFaktorenPflicht` in
 *  _publicNinePointPrompt()s Abschnitt 4 (POSITIVE MODELLFAKTOREN, nur
 *  gerendert wenn gesetzt — analog zum bestehenden
 *  o.kriterienDifferenzierungText-Muster in Abschnitt 3), befuellt NUR
 *  bei atmna mit einem Pflicht-Satz, der BB-Position und Tightness als
 *  eigenstaendigen Teil der Modellfaktoren-Argumentation einfordert.
 *  Wirkt NICHT auf die anderen vier Options-Strategien (Hook bleibt dort
 *  ungesetzt) und NICHT auf den EIC-Zweig (_eicMasterPrompt() hat eine
 *  andere Struktur, Befund betraf ausschliesslich den Public-Digest-
 *  Pfad). Noch KEIN Live-Test durchgefuehrt.
 *
 *  Version: 2.53.26 (18.09.2026) — REVIEWER-FEEDBACK ZUR ATM/NA-CSP-
 *  TAGESEMPFEHLUNG UMGESETZT (Axel-geteilte Reviewer-Einschaetzung zum
 *  Live-Output). Kernbefund: die Ausgabe war eher eine Beschreibung des
 *  Scoring-Ergebnisses als eine entscheidungsrelevante CSP-Analyse,
 *  IVP wurde textlich zu stark gewichtet (EWJ IVP 88%/Grade B neben
 *  JNJ/XOM Grade A/IVP 87%/81% ohne Einordnung als "hoechstes IVP =
 *  bester Fit" missverstaendlich), und die im STRATEGIEPRINZIP genannten
 *  Ludwig-Kriterien (Strike-Staffelung max. 5%, OI/Volumen dreistellig,
 *  Wochenoptionen) tauchten in der konkreten Kandidatenbewertung nicht
 *  mehr auf. ZWEI GEZIELTE FIXES in _publicNinePointPrompt() (gemeinsamer
 *  9-Punkte-Builder, wirkt automatisch auf ALLE Optionsstrategien
 *  csp_wheel/atmna/weekly_income/cc/collar, keine Aenderung an
 *  score_options_atmna() selbst noetig — der Score war laut Reviewer
 *  schon plausibel, das Problem lag in der Textausgabe):
 *  (1) Abschnitt 3 (Kandidatenliste): neuer PFLICHT-ZUSATZ nur bei
 *  istOptionsStrategie=true, der explizit benennt, dass die im
 *  STRATEGIEPRINZIP genannten Optionsmarkt-Auswahlkriterien im
 *  aktuellen Snapshot nicht verifiziert sind ("UIQ identifiziert den
 *  Underlying-Kandidaten, nicht die konkret handelbare Option") — schliesst
 *  die vom Reviewer benannte Luecke zwischen Prinzip-Text und konkreter
 *  Bewertung, ohne den allgemeinen Broker-Hinweis in Abschnitt 8 zu
 *  duplizieren.
 *  (2) NEUER REASONING-GUARDRAIL i) "KEINE EINZELKRITERIUMS-DOMINANZ BEI
 *  DIVERGIERENDEN SIGNALEN" — generisch fuer alle 14 Strategien (nicht nur
 *  Options), verbietet es, ein einzelnes Kriterium (z.B. IVP) implizit als
 *  Gesamtranking darzustellen, wenn ein anderes genanntes Kriterium (z.B.
 *  Grade) bei denselben Titeln eine andere Rangfolge zeigt — verlangt
 *  stattdessen explizite Benennung der Divergenz. Noch KEIN Live-Test
 *  durchgefuehrt (naechster CSP/ATM-NA- oder csp_wheel-Lauf pruefen).
 *  Axel-Entscheidung (18.09.2026): kurzer Zwischensprint neben der
 *  eigentlich laufenden Prioritaet (Fair-Value-Engine Schritt 2), da die
 *  Guardrail-Technik direkt aus dem 04./05.09.-Equity-Sprint uebertragbar
 *  war; Deployment durch Axel erst am Folgetag.
 *
...
`;
  ...
  function _deterministicOptionsFactBlock(stratId, o) {
    var lines = [];
    // NEU (v2.53.30, 21.09.2026, LLM-Auswahl-Drift-Fix — Live-Fund
    // 20.09.2026, zweifach reproduziert: top3Syms wurde korrekt berechnet
    // und korrekt uebergeben (per Diagnose-Logging in generate_public_
    // recommendations.js v1.9 bewiesen), aber das Modell ersetzte in
  ...
  // NEU (v2.53.30, 21.09.2026, LLM-Auswahl-Drift-Fix): extrahiert die in
  // Abschnitt 3 fett markierten Ticker ("**JNJ**, **CSCO**, **MDT**" —
  // PFLICHT-FORMULIERUNG laut Abschnitt-3-Instruktion oben, s. dort). Kein
  // Treffer fuer Abschnitt 3/4-Grenze = null (kein erkennbares 9-Punkte-
  // Format, Check wird uebersprungen statt falsch positiv zu warnen —
  // gleiches Prinzip wie scanForTickerScopeViolations() in ko-ai.js).
  //
  // KRITISCHER FIX (v2.54.2, 22.09.2026, ROOT CAUSE fuer den seit v2.53.30
  // (21.09.2026) DURCHGEHEND fehlschlagenden "top3-ticker-konsistenz"-Check
  // — vierter Live-Test-Anlauf, erster mit tatsaechlichem Diagnose-Logging
  // (generate_public_recommendations.js v1.17), dadurch erstmals wirklich
  // belegt statt vermutet): das Modell schreibt Markdown-Ueberschriften
  // standardmaessig KOMPLETT fett, INKLUSIVE der Abschnittsnummer —
  // "**1. MARKT-/REGIME-KONTEXT**", nicht "1. MARKT-/REGIME-KONTEXT". Der
  // bisherige Regex `(?:^|\n)\s*3\.` erwartet nach dem Zeilenumbruch
  // AUSSCHLIESSLICH Whitespace vor der Ziffer — `\s*` matcht kein `*`
  // (kein Whitespace-Zeichen), wodurch der Match bei "**3. HÖCHSTE ...**"
  // IMMER scheiterte, unabhaengig davon, ob die Ticker im Abschnitt selbst
  // korrekt fett markiert waren. Erklaert die "Abschnitt-3-Grenze
  // gefunden: false"-Diagnose in JEDEM geloggten Fall (Erstversuch UND
  // Repair, alle fuenf Options-Strategien) — die Ticker-Extraktion wurde
  // nie erreicht, weil bereits die Abschnittsgrenzen-Erkennung scheiterte.
  // Sehr wahrscheinlich der URSPRUENGLICHE, primaere Bug seit Einfuehrung
  // dieses Validators (21.09.2026) — der v2.54.1-Fettdruck-Fix fuer Ticker
  // war berechtigt und notwendig, konnte aber gar nicht wirken, da die
  // Funktion vorher scheitert. FIX: Regex toleriert jetzt 0-2 Sternchen
  // (optionale Markdown-Fettung der GESAMTEN Ueberschriftenzeile) direkt
  // vor der Abschnittsnummer, sowohl fuer die Start- als auch die Ende-
  // Grenze (Abschnitt 4). Bewusst als Regex-Robustheit statt als weitere
  // Prompt-Anweisung geloest ("schreibe die Ueberschrift NICHT fett") —
  // dem Modell zu verbieten, Markdown-Ueberschriften markdown-typisch zu
  // formatieren, waere ein fragiler Kampf gegen dessen Standardverhalten
  // (dieselbe Lehre wie bei zahlreichen anderen Guardrails in dieser
  // Datei: eine deterministische Pruefung, die mit der natuerlichen
  // Modellausgabe umgehen kann, ist robuster als eine weitere Verhaltens-
  // vorschrift). Noch KEIN erneuter Live-Test.
  function _extractSection3Tickers(text) {
    var m = text.match(/(?:^|\n)\s*\*{0,2}\s*3\.[^\n]*\n([\s\S]*?)(?=\n\s*\*{0,2}\s*4\.)/);
    if (!m) return null;
    var matches = m[1].match(/\*\*([A-Z][A-Z0-9.\-]{0,6})\*\*/g) || [];
    return matches.map(function(x) { return x.replace(/\*\*/g, ''); });
  }

  // ERWEITERT (v2.53.30, 21.09.2026, LLM-Auswahl-Drift-Fix — Live-Fund
  // 20.09.2026, zweifach reproduziert bei atmna: top3Syms/Faktor-Block
  // nachweislich korrekt uebergeben, s. generate_public_recommendations.js
  // v1.9-Diagnose-Logging, aber das Modell ersetzte MRK durch CSCO in
  // Abschnitt 3 UND begruendete aktiv, warum MRK "die Kriterien nicht
  // erfuellt" — ein reiner Vorhanden/Fehlt-Check auf Bollinger/Tightness
  // haette das NICHT erkannt, da diese Formulierungen fuer JNJ/MDT weiter
  // vorhanden waren). Neuer optionaler 3. Parameter `options.expectedTop3`
  // (Array von Symbolen, i.d.R. dasselbe top3Syms, das auch den
  // deterministischen Faktor-Block gespeist hat) — bei Angabe wird ein
  // Mengenvergleich (nicht Reihenfolge-sensitiv, Reviewer-Vorgabe) gegen
  // die tatsaechlich in Abschnitt 3 genannten Ticker durchgefuehrt.
  // RUECKWAERTSKOMPATIBEL: wird options/expectedTop3 nicht uebergeben
  // (bestehende Aufrufer, z.B. eine noch nicht aktualisierte ko-ai.js-
  // Kopie), wird dieser Check komplett uebersprungen — exakt das bisherige
  // Verhalten bleibt erhalten.
  function _validateBriefingCompliance(stratId, outputText, options) {
    var missing = [];
    var text = (outputText || '');
    var details = {};
    ...
    if (_OPTIONS_STRATEGY_IDS.indexOf(stratId) !== -1
      ...
      var expected = options.expectedTop3.slice();
      var actual = _extractSection3Tickers(text) || [];
      var expectedSet = {};
      expected.forEach(function(s) { expectedSet[s] = true; });
      var actualSet = {};
      actual.forEach(function(s) { actualSet[s] = true; });
      var missingTickers = expected.filter(function(s) { return !actualSet[s]; });
      var unexpectedTickers = actual.filter(function(s) { return !expectedSet[s]; });
      if (missingTickers.length || unexpectedTickers.length) {
        missing.push('top3-ticker-konsistenz');
        details.top3Mismatch = {
          expected: expected, actual: actual,
          missingTickers: missingTickers, unexpectedTickers: unexpectedTickers,
        };
      }
    }

    var result = { status: missing.length ? 'FAIL' : 'PASS', missing: missing };
  ...
  // NEU (v2.54.4, 23.09.2026, Backlog-Punkt vom 13.09.2026 — "Boilerplate-
  // Abschnitte 7+8 sind grossteils statisch... sollen app-seitig templatet
  // statt vom LLM erzeugt werden — spart Input+Output-Token"): Abschnitt 7
  // ("Was UIQ ableiten kann") und Abschnitt 8 ("Modell-Grenze") empirisch
  // geprueft (23.09.2026, alle 15 Strategien, je zwei stark unterschiedliche
  // Tageskontexte) — beide sind byte-identisch unabhaengig von
  // ctx.marktkontext/ctx.top3Syms. Einzige Eingaben: o.stratName,
  // o.modellGrenzeText, o.istOptionsStrategie — alles strategie-konstante
  // Werte, keine Tagesdaten. Extrahiert aus der bisherigen Inline-Logik in
  // _publicNinePointPrompt() OHNE inhaltliche Aenderung (Wortlaut, Pflicht-
  // Satzmuster, alle bisherigen Guardrail-Klauseln 1:1 uebernommen) — reine
  // Verschiebung in eine benannte, separat aufrufbare Funktion, damit sie
  // sowohl beim Prompt-Bau (unveraendertes Verhalten, Default) als auch
  // POST-HOC durch den aufrufenden Code (generate_public_recommendations.js,
  // neues opt-in ueber ctx.skipAbschnitt78) genutzt werden kann, ohne die
  // Guardrail-Texte doppelt zu pflegen (Multiple-Sources-of-Truth-Risiko
  // vermeiden, s. wiederholte Faelle dieser Art in der Aenderungshistorie).
  function _buildAbschnitt78(o) {
    var istOptionsLocal = !!o.istOptionsStrategie;
    var modellGrenzeZusatz = istOptionsLocal
    ...
    var abschnitt8Local = '8. WAS UIQ NICHT ABLEITEN KANN ("Modell-Grenze"): wenn der '
    ...
    var abschnitt7Local = '7. WAS UIQ ABLEITEN KANN: EIN kurzer, präziser Satz, '
    ...
    return { abschnitt7: abschnitt7Local, abschnitt8: abschnitt8Local, combined: abschnitt7Local + abschnitt8Local };
  }

  function _publicNinePointPrompt(ctx, o) {
    var mode = o.mode || 'scan';
    var istOptions = !!o.istOptionsStrategie;
    // NEU (v2.54.4, 23.09.2026): opt-in Flag, Default-Verhalten (Flag nicht
    // gesetzt) bleibt fuer JEDEN bestehenden Aufrufer 100% unveraendert —
    // Abschnitt 7+8 werden wie bisher inline in den Prompt geschrieben und
    // vom Modell selbst generiert. Nur wenn ctx.skipAbschnitt78 === true
    // explizit gesetzt ist (aktuell nur von
    // generate_public_recommendations.js genutzt), entfaellt Abschnitt 7+8
    // im Prompt UND in der erwarteten Modell-Ausgabe — der aufrufende Code
    // fuegt sie stattdessen selbst aus KoPrompts.buildAbschnitt78() hinzu.
    var skipAbschnitt78 = ctx && ctx.skipAbschnitt78 === true;
    ...
    if (mode === 'holding_review') {
      ...
      abschnitt3 = '3. TITEL MIT MODELLBASIERTEM ABSICHERUNGS-HINWEIS: '
        ...
        + 'sinngemäß, JEDES Tickersymbol dabei in Markdown-Fettdruck '
        + '(doppelte Sternchen, z.B. **LMT**) — dies ist technisch '
        + 'erforderlich, damit eine automatisierte Prüfung die genannten '
        + 'Titel gegen die vorgegebene Liste abgleichen kann, kein '
        + 'gestalterischer Vorschlag: "Folgende Titel erfüllen die '
        + 'definierten Modellkriterien für eine Absicherungsüberprüfung '
        + '(Reihenfolge ohne Wertung): **[Titel 1]**, **[Titel 2]**, '
        + '**[Titel 3]**."\n';
    } else {
      ...
      abschnitt3 = '3. Überschrift EXAKT "HÖCHSTE ' + o.stratName.toUpperCase() + ' STRATEGY-FITS" '
        ...
        + (istOptions
            ? ('sinngemäß, JEDES Tickersymbol dabei in Markdown-Fettdruck '
               + '(doppelte Sternchen, z.B. **AAPL**) — dies ist technisch '
               + 'erforderlich, damit eine automatisierte Prüfung die '
               + 'genannten Titel gegen die vorgegebene Liste abgleichen '
               + 'kann, kein gestalterischer Vorschlag: "Folgende '
               + 'Titel weisen im betrachteten Snapshot den höchsten Strategy '
               + 'Fit auf (Rangfolge gemäß UIQ-Kriterien-Score, keine '
               + 'Anlageempfehlung): **[Titel 1]**, **[Titel 2]**, '
               + '**[Titel 3]**." ')
    ...
    var abschnitt4 = '4. POSITIVE MODELLFAKTOREN: EIN gemeinsamer Absatz, der '
      ...
      + (istOptions
          ? (' BEGRIFFS-KLARSTELLUNG PFLICHT (18.09.2026, Reviewer-Fund '
             + 'ATM/NA-Live-Test, gilt für JEDE Optionsstrategie): Falls IVP '
             + 'bzw. ivpPercentile genannt wird, MUSS es als echtes Implied-'
             + 'Volatility-Perzentil bezeichnet werden ("IV-Perzentil", '
             + '"implizite Volatilität") — NIEMALS als "realisierte '
             + 'Volatilität" oder "historische Volatilität" (belegter Fund: '
             + '"zeigen im historischen Vergleich erhöhte realisierte '
             + 'Volatilität" — sachlich falsch, ivpPercentile stammt aus '
             + 'echten IV-Daten, ist kein HVP-Proxy). Liegt stattdessen nur '
             + 'HVP vor (Historical Volatility Percentile, Fallback-Feld), '
             + 'MUSS es explizit als "HVP"/"historische Volatilität" '
             + 'gekennzeichnet werden — beide Begriffe NIEMALS synonym '
             + 'verwenden. ZUSÄTZLICH: der berechnete Strike-Näherungswert '
             + '(EMA200-1,5×ATR) ist ein modelliertes Referenzniveau der '
             + 'Aktie, KEINE Rollschwelle — NIEMALS "Puffer zur Rollschwelle" '
             + 'oder "Rollschwelle" für diesen Wert verwenden (belegter Fund: '
             + '"breiter definierter Puffer zur Rollschwelle" — die '
             + 'tatsächliche Rollentscheidung hängt laut STRATEGIEPRINZIP von '
             + 'Zeit bis Verfall, ITM-Situation und Prämienökonomie ab, NICHT '
             + 'vom EMA200-Abstand). STATTDESSEN: "modelliertes '
    ...
    var modellGrenzeZusatz = istOptions
      ? (' Zusätzlich: keine konkrete Strike-, Delta-, DTE-, Prämien- oder '
         ...
         + 'sind außerhalb von UIQ im Broker zu prüfen.')
      : ' Zusätzlich: Einstiegszeitpunkt, Positionsgröße und individuelle Risikolage sind außerhalb von UIQ zu prüfen.';
    // GEAENDERT (v2.54.4, 23.09.2026): die bisher hier duplizierte Logik
    // (modellGrenzeZusatz-Variable oben bleibt fuer Rueckwaertskompatibilitaet
    // stehen, wird aber nicht mehr direkt fuer abschnitt7/8 verwendet — s.
    // _buildAbschnitt78() oben, identischer Wortlaut, nur ausgelagert) durch
    // einen einzigen Aufruf ersetzt, um die Guardrail-Texte nicht doppelt zu
    // pflegen.
    var abschnitt78Parts = _buildAbschnitt78(o);
    var abschnitt7 = skipAbschnitt78 ? '' : abschnitt78Parts.abschnitt7;
    var abschnitt8 = skipAbschnitt78 ? '' : abschnitt78Parts.abschnitt8;
    ...
    var fullPrompt = KI_ANTI_HALLUZINATION
      + PUBLIC_REGULATORY_GUARDRAIL
      + '⚠️ Diese Analyse ist eine statistische Kontext-Analyse gem. §1 WpHG — '
      + 'keine Anlageberatung, keine Kauf-/Verkaufsempfehlung. Es werden '
      ...
      + 'verworfenes Risiko.\n'
      + 'i) KEINE EINZELKRITERIUMS-DOMINANZ BEI DIVERGIERENDEN SIGNALEN '
      + '(18.09.2026, Reviewer-Feedback zur ATM/NA-CSP-Tagesempfehlung): wenn '
      + 'die in Abschnitt 3 genannten Titel bei einem Kriterium (z.B. IVP) '
      + 'NICHT dieselbe Rangfolge zeigen wie bei einem anderen Kriterium '
      + '(z.B. Grade), darf das eine Kriterium NIEMALS implizit als '
      + 'Gesamt-Rangfolge dargestellt werden, nur weil es zuerst genannt '
      + 'oder zahlenmäßig am höchsten ist. BELEGTES BEISPIEL: ein Titel mit '
      + 'IVP 88% (Grade B) neben zwei Titeln mit niedrigerem IVP (87%/81%) '
      + 'aber Grade A OHNE Einordnung nebeneinanderzustellen erzeugt beim '
      + 'Leser den falschen Eindruck "höchstes IVP = bester Strategy-Fit" — '
      + 'obwohl das Modell selbst über den Grade ein schwächeres Underlying-'
      + 'Fit-Signal für diesen Titel ausweist. STATTDESSEN: Divergenz '
      + 'zwischen genannten Kriterien bei denselben Titeln explizit '
      + 'benennen (z.B. "X weist im Snapshot das höchste IVP auf, erreicht '
      + 'jedoch nur Grade B; Y und Z kombinieren Grade A mit einem etwas '
      + 'niedrigeren IVP") statt Titel nur nebeneinander mit ihren '
      + 'Einzelwerten aufzuzählen. Gilt für jedes Kriterienpaar, nicht nur '
      + 'IVP/Grade, und für jede Strategie, nicht nur Optionsstrategien. '
      + 'ERGÄNZUNG (18.09.2026, belegter Fund ATM/NA-Live-Test — Regel b '
      + 'gilt UNVERÄNDERT auch hier): "teilt"/"gemeinsam"-Sprache ist '
      + 'AUSSCHLIESSLICH echten Zahlen-Gleichständen vorbehalten. Belegter '
      + 'Fund: "PJP teilt mit JNJ den höchsten IVP-Bereich (84%ile)" bei '
      + 'JNJ IVP 87%ile — das sind ZWEI VERSCHIEDENE Werte, kein Gleichstand, '
      + 'auch wenn beide "hoch" sind. Bei unterschiedlichen Werten IMMER '
      + 'den jeweils höheren/niedrigeren einzeln benennen (z.B. "JNJ weist '
      + 'mit 87%ile das höchste IVP auf, PJP liegt mit 84%ile knapp '
      + 'darunter"), NIEMALS "teilt"/"gemeinsam" bei nur ähnlicher '
      + 'Größenordnung.\n\n'
      ...
      + abschnitt6
      + (skipAbschnitt78
          ? ('\n[Abschnitt 7 "Was UIQ ableiten kann" und Abschnitt 8 '
             + '"Modell-Grenze" werden NICHT von dir geschrieben — sie werden '
             + 'automatisch, unverändert und deterministisch ergänzt. Fahre '
             + 'nach Abschnitt 6 DIREKT mit Abschnitt 9 fort, nummeriere ihn '
             + 'trotzdem als "9." (nicht als "7."), damit die Nummerierung im '
             + 'veröffentlichten Text durchgängig 1-9 bleibt.]\n')
          : '')
      + abschnitt7
      + abschnitt8
      + abschnitt9
      + '\nSCHLUSS-SELBSTPRÜFUNG (05.09.2026, PFLICHT vor Abgabe der Antwort, '
      + 'ZWEI SCHRITTE — bei Optionsstrategien zusätzlich SCHRITT 3, '
      + '18.09.2026):\n'
      ...
      + 'SATZ der Abschnitte ' + (skipAbschnitt78 ? '4, 5, 6 und 9' : '4 bis 9') + ' Wort für Wort auf Ticker-Nennungen — '
      ...
      + 'genannt ist.\n'
      + (istOptions
          ? ('SCHRITT 3 (PFLICHT-ZUSÄTZE OPTIONSSTRATEGIEN, 18.09.2026, '
             + 'ERWEITERT 22.09.2026 um Prüfpunkt (c) — '
             + 'BELEGTER WIEDERHOLUNGSFUND: im ersten Live-Test NACH '
             + 'Einführung dieser Pflicht-Zusätze fehlten BEIDE ersatzlos, '
             + 'obwohl Abschnitt 3/4 sie ausdrücklich verlangen — Regel-'
             + 'Existenz allein reicht nicht, explizite Vor-Abgabe-Prüfung '
             + 'nötig, analog zu Schritt 1/2 oben): geht die fertige Antwort '
             + 'durch und prüft explizit, ob (a) Abschnitt 3 den Satz zum '
             + 'nicht verifizierten Optionsmarkt-Validierungsstatus enthält '
             + '(Strike-Staffelung/OI-Volumen/Wochenoptionen — s. Abschnitt-3-'
             + 'Vorgabe oben) UND (b) — NUR falls für diese Strategie '
             + 'vorgegeben — Abschnitt 4 die dort verlangten strategie-'
             + 'spezifischen Kennzahlen (z.B. bei atmna: Bollinger-Position '
             + 'UND Tightness) explizit als eigene Ranking-Begründung nennt, '
             + 'nicht nur implizit im Ticker-Kontext stehen lässt, UND (c) '
             + 'JEDES in Abschnitt 3 genannte Tickersymbol tatsächlich in '
             + 'Markdown-Fettdruck steht (z.B. **AAPL**, nicht nur AAPL) — '
             + 'belegter Fund 22.09.2026, erster echter Live-Test des '
             + 'gesamten Compliance-Mechanismus: die Fettdruck-Anforderung '
             + 'wurde in KEINEM von fünf parallelen Läufen befolgt, obwohl '
             + 'Abschnitt 3 sie explizit vorschreibt (s. dort) — eine '
             + 'automatisierte Prüfung liest NUR fett markierte Tickersymbole '
             + 'aus Abschnitt 3 aus, ein unformatiert genannter Ticker gilt '
             + 'für diese Prüfung als NICHT genannt, selbst wenn der Name '
             + 'korrekt im Text steht. Fehlt einer der drei Pflicht-Zusätze, '
             + 'MUSS er vor Abgabe ergänzt werden — NICHT als nachträgliche '
             + 'Ergänzung nach Abschnitt 9, sondern an der dafür vorgesehenen '
             + 'Stelle in Abschnitt 3/4.\n')
          : '')
    ...
    // NEU (v2.54.4, 23.09.2026): nur bei gesetztem skipAbschnitt78-Flag
    // aendert sich die RUECKGABEFORM (Objekt statt String) — jeder
    // bestehende Aufrufer, der das Flag nicht setzt, bekommt exakt wie
    // bisher einen reinen String zurueck, keine Verhaltensaenderung.
    if (skipAbschnitt78) {
      return {
        prompt: fullPrompt,
        abschnitt7: abschnitt78Parts.abschnitt7,
        abschnitt8: abschnitt78Parts.abschnitt8,
      };
    }
    return fullPrompt;
  }
...
;
  ...
  const STRATEGIES = {
    ...
    prompt: function(ctx) {
        ...
        // ERGAENZT (18.09.2026, ATMNA-Explainability-Gap-Fix — Backlog-Fund
        // aus generate_public_recommendations.js v1.5, Thema-3-Abschlusstest
        ...
        // Kontext stehen, statt sie als eigene Ranking-Begruendung
        // einzufordern — las sich wie eine wiederverwendete CSP-Begruendung.
        // Axel-Entscheidung 16.09.2026: bewusst zurueckgestellt bis zur
        // naechsten gezielten Options-Prompt-Qualitaetsrunde — das ist diese
        // hier (18.09.2026, direkt im Anschluss an den Reviewer-Fix).
        var atmnaExpliziteFaktorenText = 'ATMNA-SPEZIFISCHER PFLICHT-ZUSATZ '
          + '(18.09.2026, Explainability-Gap-Fix, Backlog-Fund aus '
          + 'generate_public_recommendations.js): dieser Absatz MUSS '
          + 'explizit auf die Bollinger-Position (BB, Naehe zur Mittelzone) '
          + 'UND die Kurs-Tightness (Tightness) der in Abschnitt 3 genannten '
          + 'Titel eingehen — diese beiden Kennzahlen sind die fuer CSP '
          + '(ATM/NA) charakteristischen, von den uebrigen Options-'
          + 'Strategien abweichenden Kriterien (Naehe zum ATM-Zeitwert-'
          + 'Maximum bzw. Konsolidierungsstabilitaet fuer die Roll-Eignung) '
          + 'und duerfen NICHT nur implizit in den allgemeinen Faktoren '
          + '(RSI/Grade/D200-Abstand/IVP) untergehen. Eine Begruendung, die '
          + 'BB-Position und Tightness gar nicht oder nur beilaeufig nennt, '
          + 'erfuellt diese Vorgabe NICHT — beide muessen als eigenstaendiger '
          + 'Teil der Modellfaktoren-Argumentation erscheinen, nicht nur als '
          + 'Zahlenwert im Ticker-Kontext stehen bleiben.';
        if (!ctx.isEic) {
          return _publicNinePointPrompt(ctx, {
    ...
    value: {
      ...
      prompt: function(ctx) {
        ...
        if (!ctx.isEic) {
          // FIX (v2.54.4, 23.09.2026): vorher wurde hier ein komplett NEUES
          // ctx-Objekt gebaut (nur marktkontext), wodurch JEDES andere
          // ctx-Feld (u.a. das neue skipAbschnitt78) verloren ging, bevor es
          // _publicNinePointPrompt() erreichte — Live-Fund beim Testen des
          // Abschnitt-7+8-Templatings. Jetzt: Original-ctx per Object.assign
          // durchreichen, nur marktkontext gezielt ueberschreiben.
          return _publicNinePointPrompt(Object.assign({}, ctx, { marktkontext: _marktkontextMitTickern }), {
  ...
  const KoPrompts = {
    ...
    // NEU (v2.54.3, 23.09.2026, Prompt-Caching-Vorarbeit): bislang intern,
    // jetzt exportiert — zusammen mit KI_ANTI_HALLUZINATION der einzige
    // Textblock, der ueber ALLE 15 Strategien (Public-Modus) hinweg
    // byte-identisch am ANFANG jedes Prompts steht (empirisch verifiziert:
    // gemeinsames Praefix ueber alle 15 Strategien = 28.854 Zeichen /
    // ~7.214 Token, deutlich ueber der Cache-Mindestgroesse). Grund fuer
    // den Export: generate_public_recommendations.js braucht eine robuste,
    // NAMENTLICHE (nicht zeichenzahl-basierte) Grenze, um den Prompt fuer
    // Anthropics Prompt-Caching (cache_control) in einen cachebaren
    // Praefix und einen taeglich wechselnden Rest zu splitten, ohne dass
    // sich diese Grenze bei jeder kuenftigen Wortlaut-Aenderung an einer
    // einzelnen Strategie verschieben kann. S. SHARED_STATIC_PREFIX unten
    // fuer die fertige Kombination.
    PUBLIC_REGULATORY_GUARDRAIL,

    /**
     * Der cachebare, ueber ALLE 15 Strategien (Public-Modus) identische
     * Prompt-Praefix. NEU (v2.54.3, 23.09.2026). Verwendung im aufrufenden
     * Code: prueft `KoPrompts.get(stratId, ctx).startsWith(KoPrompts.SHARED_STATIC_PREFIX)`
     * und splittet bei Erfolg den Prompt in zwei content-Bloecke (Praefix
     * mit cache_control, Rest ohne) statt einer einzelnen Zeichenkette —
     * s. UIQ-Suite/scripts/generate_public_recommendations.js::
     * buildCacheableContent(). Bei fehlendem Praefix-Treffer (z.B. kuenftige Strategie ohne diesen
     * Block) faellt der Aufrufer defensiv auf den unveraenderten
     * Einzelstring zurueck — kein Hard-Fail.
     */
    SHARED_STATIC_PREFIX: KI_ANTI_HALLUZINATION + PUBLIC_REGULATORY_GUARDRAIL,
     ...
     */
    validateBriefingCompliance(stratId, outputText, options) {
      return _validateBriefingCompliance(stratId, outputText, options);
    },
